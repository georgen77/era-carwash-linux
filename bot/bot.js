...
// ── PHOTO LIBRARY ───────────────────────────────────────────────────────────
var PHOTOS_FILE = __dirname + '/photos.json';
var CATEGORIES_FILE = __dirname + '/photo_categories.json';
var DEFAULT_CATEGORIES = [
  { id: 'apartments', name: 'Апартаменты', emoji: '🏠' },
  { id: 'linen',      name: 'Бельё',       emoji: '🧺' },
  { id: 'repair',     name: 'Ремонт',       emoji: '🔧' },
  { id: 'documents',  name: 'Документы',    emoji: '📄' },
  { id: 'receipts',   name: 'Чеки',         emoji: '🧾' }
];
function loadCategories() {
  try { if (fs.existsSync(CATEGORIES_FILE)) return JSON.parse(fs.readFileSync(CATEGORIES_FILE, 'utf8')); } catch(e) {}
  fs.writeFileSync(CATEGORIES_FILE, JSON.stringify(DEFAULT_CATEGORIES, null, 2));
  return DEFAULT_CATEGORIES.slice();
}
function saveCategories(cats) { fs.writeFileSync(CATEGORIES_FILE, JSON.stringify(cats, null, 2)); }
function loadPhotos() {
  try { if (fs.existsSync(PHOTOS_FILE)) return JSON.parse(fs.readFileSync(PHOTOS_FILE, 'utf8')); } catch(e) {}
  return [];
}
function savePhotos(photos) { fs.writeFileSync(PHOTOS_FILE, JSON.stringify(photos, null, 2)); }
function addPhotoRecord(data) {
  var photos = loadPhotos();
  var rec = Object.assign({ id: Date.now() + '_' + Math.random().toString(36).slice(2,7), created_at: new Date().toISOString(), categories: [], caption: '', apartment: '' }, data);
  photos.push(rec);
  savePhotos(photos);
  return rec;
}
function buildCatKeyboard(selected) {
  var cats = loadCategories();
  var rows = [];
  for (var i = 0; i < cats.length; i += 2) {
    var row = [];
    [cats[i], cats[i+1]].forEach(function(cat) {
      if (!cat) return;
      row.push({ text: (selected.includes(cat.id) ? '✅ ' : '⬜ ') + cat.emoji + ' ' + cat.name, callback_data: 'ptag_' + cat.id });
    });
    rows.push(row);
  }
  rows.push([{ text: '➕ Новая категория', callback_data: 'pcat_new' }, { text: '💾 Сохранить', callback_data: 'psave' }]);
  return rows;
}
...
async function notifyLockCode(result, booking) {
  ...
  var guestUrl = buildGuestPortalUrl(result.apartment, result.codeDisplay, checkin, checkout, true);
  var guestUrlForGuest = buildGuestPortalUrl(result.apartment, result.codeDisplay, checkin, checkout, false);
  var text = '🔐 *Код для гостей*\n\n';
  text += '🏠 *' + apt + '*\n';
  text += '📅 ' + fmtDateShort(result.checkin) + ' → ' + fmtDateShort(result.checkout) + '\n\n';
  text += '🔑 *Код: ' + result.codeDisplay + '#*\n\n';
  text += '⏰ Действует:\n';
  text += '  С: ' + result.validFrom + '\n';
  text += '  По: ' + result.validTo + '\n\n';
  text += '📤 *Ссылка для гостя:*\n`' + guestUrlForGuest + '`\n\n';
  ...
  var inlineButtons = [
    [
      { text: '👁 Превью портала', url: guestUrl },
      { text: '📤 Портал гостя', url: guestUrlForGuest }
    ],
    [
  ...
  for (var adminId of [OWNER_CHAT_ID, IRINA_CHAT_ID, EMMA_CHAT_ID]) {
    if (adminId) try { await bot.sendMessage(adminId, text, { parse_mode: 'Markdown', reply_markup: { inline_keyboard: inlineButtons } }); } catch(e) {}
  }
}
...
const GUEST_PORTAL_BASE = 'https://era-group.world/era/guest.html';
function buildGuestPortalUrl(apt, code, checkinDate, checkoutDate, isAdmin) {
  ...
  return GUEST_PORTAL_BASE + '?' + params.toString();
}
  ...
  try {
    ...
    if (isAdmin(chatIdStr)) {
      ...
      upcoming.forEach(function(s) {
        ...
        // Заголовок смены — компактный формат без слов Заезд/Выезд
        var cleanerFirst = cleanerFull.split(' ')[0];
        var headerParts = [
          (isToday ? '🔴 ' : '') + aptShort,
          '📅 '+fmtDateShort(s.checkin_date||s.checkout_date)+'→'+fmtDateShort(s.checkout_date),
          '👥'+guests+'г · 🧹'+cleanerFirst,
          src || '',
        ].filter(Boolean).join(' · ');
...
bot.on('callback_query', async function(query) {
  ...
  // ── PHOTO LIBRARY CALLBACKS ──────────────────────────────────────────────
  if (data === 'photo_is_receipt') {
    var sess = sessions[chatId];
    if (!sess) return bot.answerCallbackQuery(query.id);
    sessions[chatId] = { step: 'expense_receipt', amount: null, description: '', mappedCat: 'Прочее', photoFileId: sess.photoFileId };
    bot.editMessageText('🧾 Хорошо, обработаю как чек.\n\n_Нажми кнопку ниже или введи сумму вручную:_', {
  ...
  if (data === 'photo_is_archive') {
    var sess = sessions[chatId];
    if (!sess) return bot.answerCallbackQuery(query.id);
    sessions[chatId] = { step: 'photo_categorize', photoFileId: sess.photoFileId, caption: sess.caption || '', selectedCategories: [], url: null };
    bot.editMessageText('📁 Выбери категории (можно несколько):', {
      chat_id: chatId, message_id: query.message.message_id,
      reply_markup: { inline_keyboard: buildCatKeyboard([]) }
    });
    bot.answerCallbackQuery(query.id);
    return;
  }

  if (data.startsWith('ptag_')) {
    var sess = sessions[chatId];
    if (!sess || sess.step !== 'photo_categorize') return bot.answerCallbackQuery(query.id);
    var catId = data.slice(5);
    var idx = sess.selectedCategories.indexOf(catId);
    if (idx >= 0) sess.selectedCategories.splice(idx, 1); else sess.selectedCategories.push(catId);
    bot.editMessageReplyMarkup({ inline_keyboard: buildCatKeyboard(sess.selectedCategories) }, { chat_id: chatId, message_id: query.message.message_id });
    bot.answerCallbackQuery(query.id);
    return;
  }
  ...
  if (data === 'pcat_new') {
    var sess = sessions[chatId];
    if (!sess || sess.step !== 'photo_categorize') return bot.answerCallbackQuery(query.id);
    sess.pendingNewCat = true;
    bot.sendMessage(chatId, '➕ Введи название новой категории с эмодзи:\n_Например: 🚗 Машина_', { parse_mode: 'Markdown' });
    bot.answerCallbackQuery(query.id);
    return;
  }
  ...
  if (data === 'psave') {
    var sess = sessions[chatId];
    if (!sess || sess.step !== 'photo_categorize') return bot.answerCallbackQuery(query.id);
    bot.answerCallbackQuery(query.id, { text: '💾 Сохраняю...' });
    try {
      var url = sess.url;
      if (!url && sess.photoFileId) {
        var fi = await bot.getFile(sess.photoFileId);
        var fu = 'https://api.telegram.org/file/bot' + TOKEN + '/' + fi.file_path;
        var buf = Buffer.from((await axios.get(fu, { responseType: 'arraybuffer' })).data);
        url = await uploadReceiptToStorage(buf, 'archive_' + chatId + '_' + Date.now() + '.jpg');
      }
      if (sess.editingId) {
        var photos = loadPhotos();
        var pi = photos.findIndex(function(p){ return p.id === sess.editingId; });
        if (pi >= 0) { photos[pi].categories = sess.selectedCategories; savePhotos(photos); }
      } else {
        addPhotoRecord({ telegram_file_id: sess.photoFileId, url: url, categories: sess.selectedCategories, caption: sess.caption, uploaded_by: chatIdStr });
      }
      var cats = loadCategories();
      var catNames = sess.selectedCategories.map(function(id){ var c = cats.find(function(x){ return x.id===id; }); return c ? c.emoji+' '+c.name : id; }).join(', ') || 'Без категории';
      bot.editMessageText('✅ Сохранено!\n📁 ' + catNames, { chat_id: chatId, message_id: query.message.message_id });
    } catch(e) { bot.sendMessage(chatId, '❌ Ошибка: ' + e.message); }
    sessions[chatId] = null;
    return;
  }

  if (data.startsWith('pfilter_')) {
    var filter = data.slice(8);
    var photos = loadPhotos();
    var filtered = filter === 'all' ? photos : photos.filter(function(p){ return p.categories && p.categories.includes(filter); });
    filtered = filtered.slice().reverse().slice(0, 10);
    bot.answerCallbackQuery(query.id);
    if (!filtered.length) { bot.sendMessage(chatId, '📭 Фото не найдено'); return; }
    bot.sendMessage(chatId, '📸 Найдено ' + filtered.length + ' фото (последние):');
    var cats = loadCategories();
    for (var pi = 0; pi < filtered.length; pi++) {
      var p = filtered[pi];
      var catNames = (p.categories||[]).map(function(id){ var c=cats.find(function(x){return x.id===id;}); return c?c.emoji+c.name:id; }).join(' · ');
      var cap = [catNames, p.caption, p.created_at ? p.created_at.slice(0,10) : ''].filter(Boolean).join('\n');
      try {
        await bot.sendPhoto(chatId, p.telegram_file_id, { caption: cap, reply_markup: { inline_keyboard: [[{ text: '✏️ Категории', callback_data: 'pedit_'+p.id }]] }});
      } catch(e) { bot.sendMessage(chatId, '🔗 ' + (p.url||'?') + '\n' + cap); }
    }
    return;
  }

  if (data.startsWith('pedit_')) {
    var photoId = data.slice(6);
    var photos = loadPhotos();
    var photo = photos.find(function(p){ return p.id === photoId; });
    if (!photo) { bot.answerCallbackQuery(query.id, { text: 'Не найдено' }); return; }
    sessions[chatId] = { step: 'photo_categorize', photoFileId: photo.telegram_file_id, caption: photo.caption||'', selectedCategories: (photo.categories||[]).slice(), url: photo.url, editingId: photoId };
    bot.sendMessage(chatId, '✏️ Изменить категории:', { reply_markup: { inline_keyboard: buildCatKeyboard(photo.categories||[]) }});
    bot.answerCallbackQuery(query.id);
    return;
  }
...
bot.on('message', async function(msg) {
  ...
  // Новая категория для фотоархива
  if (sessions[chatId] && sessions[chatId].step === 'photo_categorize' && sessions[chatId].pendingNewCat) {
    var parts = text.split(' ');
    var emoji = parts[0]; var name = parts.slice(1).join(' ') || text;
    var cats = loadCategories();
    var newCatId = 'cat_' + Date.now();
    cats.push({ id: newCatId, name: name, emoji: emoji });
    saveCategories(cats);
    sessions[chatId].pendingNewCat = false;
    sessions[chatId].selectedCategories.push(newCatId);
    bot.sendMessage(chatId, '✅ Категория добавлена: ' + emoji + ' ' + name + '\n\nВыбери категории:', {
      reply_markup: { inline_keyboard: buildCatKeyboard(sessions[chatId].selectedCategories) }
    });
    return;
  }
...
async function parseVoiceCommand(text, scheduleData) {
  try {
    ...
    var systemPrompt =
      ...
      'ПРАВИЛА:\n' +
      '- Сегодня ' + today + '. Даты YYYY-MM-DD.\n' +
      '- Если упомянуто только ЧИСЛО без месяца (например "на 15-е", "на пятнадцатое"): если это число уже ПРОШЛО в текущем месяце — берёшь СЛЕДУЮЩИЙ месяц. Если ещё не наступило — текущий месяц.\n' +
      ...
      '{"action":"lock_create","apartment":"salvador","checkin":"2026-05-01","checkout":"2026-05-01","checkin_time":"10:00","checkout_time":"22:00","confirm_text":"Создать код для Сальвадора · 01.05 10:00 → 01.05 22:00"}\n' +
      ...
      '- "создай код для" / "сгенерируй код" → lock_create. Если нестандартное время → ОБЯЗАТЕЛЬНО добавь checkin_time/checkout_time (HH:MM). Парсить: \"с 10 утра\"=checkin_time:\"10:00\", \"с 10:00 утра\"=checkin_time:\"10:00\", \"до 5 вечера\"=checkout_time:\"17:00\", \"до 10 вечера\"=checkout_time:\"22:00\", \"до 10:00 вечера\"=checkout_time:\"22:00\", \"до часу дня\"=checkout_time:\"13:00\", \"до полудня\"=checkout_time:\"12:00\". Вечер (19-23ч): +12 к часу. По умолчанию 15:00/11:00 ТОЛЬКО если время не упомянуто. checkout_time указывает время В ДЕНЬ ВЫЕЗДА по броне\n' +
...
bot.on('photo', async function(msg) {
  ...
  // Фото без активной сессии — спрашиваем что это
  if (isAdmin(chatIdStr)) {
    var fileIdRaw = msg.photo[msg.photo.length - 1].file_id;
    sessions[chatId] = { step: 'photo_type_pending', photoFileId: fileIdRaw, caption: msg.caption || '' };
    bot.sendMessage(chatId, '📸 Что это за фото?', {
      reply_markup: { inline_keyboard: [
        [{ text: '🧾 Чек / расход', callback_data: 'photo_is_receipt' }, { text: '📁 В архив', callback_data: 'photo_is_archive' }],
        [{ text: '❌ Отмена', callback_data: 'receipt_cancel' }]
      ]}
    });
  }
...
// ── КОМАНДА /фото ──────────────────────────────────────────────────────────
bot.onText(/^\/фото/, function(msg) {
  var chatId = msg.chat.id; var chatIdStr = String(chatId);
  if (!isAdmin(chatIdStr)) return;
  var cats = loadCategories();
  var keyboard = cats.map(function(cat) {
    return [{ text: cat.emoji + ' ' + cat.name, callback_data: 'pfilter_' + cat.id }];
  });
  keyboard.push([{ text: '📸 Все фото', callback_data: 'pfilter_all' }]);
  bot.sendMessage(chatId, '📸 *Фотоархив* — выбери категорию:', { parse_mode: 'Markdown', reply_markup: { inline_keyboard: keyboard } });
});

bot.onText(/^\/категории/, function(msg) {
  var chatId = msg.chat.id; var chatIdStr = String(chatId);
  if (!isAdmin(chatIdStr)) return;
  var cats = loadCategories();
  var list = cats.map(function(c, i){ return (i+1) + '. ' + c.emoji + ' ' + c.name; }).join('\n');
  bot.sendMessage(chatId, '📁 *Категории фотоархива:*\n\n' + list + '\n\n_Чтобы добавить категорию — пришли фото и выбери "➕ Новая категория"_', { parse_mode: 'Markdown' });
});
  ...
  try {
    ...
    if (cmd.action === 'lock_create' && cmd.checkin && scheduleCache && scheduleCache.length > 0) {
      ...
      if (found) {
        ...
        var ciT = cmd.checkin_time || '15:00';
        var coT = cmd.checkout_time || '11:00';
        cmd.confirm_text = 'Создать код ' + (cmd.custom_code||'') + ' для ' + aptName2 + ' · ' + fmtDateShort(found.checkin_date) + ' ' + ciT + ' → ' + fmtDateShort(found.checkout_date) + ' ' + coT;
      }
    }
...
console.log('ERA Bot v91 запущен!');

...
async function getToken() {
  ...
  const params = new URLSearchParams({
    clientId:     clientId,
    clientSecret: clientSecret, // без MD5 хеша
    username:     username,
...
async function createPasscode(lockId, code, startDate, endDate, name) {
  ...
  const r = await axios.post(TTLOCK_BASE + '/keyboardPwd/add', params.toString(), {
    headers: { 'Content-Type': 'application/x-www-form-urlencoded' }
  });
...
async function deletePasscode(lockId, keyboardPwdId) {
  ...
  const r = await axios.post(TTLOCK_BASE + '/keyboardPwd/delete', params.toString(), {
    headers: { 'Content-Type': 'application/x-www-form-urlencoded' }
  });
...
async function getPasscodes(lockId) {
  ...
  const r = await axios.get(TTLOCK_BASE + '/keyboardPwd/list?' + params.toString());
  if (r.data.errcode && r.data.errcode !== 0) throw new Error('TTLock error ' + r.data.errcode + ': ' + r.data.errmsg);
  return r.data.list || [];
}
...
async function getLocks() {
  ...
  const r = await axios.get(TTLOCK_BASE + '/list?' + params.toString());
  if (r.data.errcode && r.data.errcode !== 0) throw new Error('TTLock error ' + r.data.errcode + ': ' + r.data.errmsg);
  return r.data.list || [];
}

async function createGuestCode(apartment, checkinDate, checkoutDate, hasSpa, customCode, customStartMs, customEndMs) {
  var code = customCode || generateCode();
  var times = getCodeTimes(checkinDate, checkoutDate);
  // Override times if custom provided
  if (customStartMs) times.startDate = customStartMs;
  if (customEndMs) times.endDate = customEndMs;
  if (customStartMs || customEndMs) {
    var s = new Date(times.startDate); var e = new Date(times.endDate);
    times.startStr = s.toISOString().slice(0,10) + ' ' + ('0'+s.getHours()).slice(-2) + ':' + ('0'+s.getMinutes()).slice(-2);
    times.endStr = e.toISOString().slice(0,10) + ' ' + ('0'+e.getHours()).slice(-2) + ':' + ('0'+e.getMinutes()).slice(-2);
  }

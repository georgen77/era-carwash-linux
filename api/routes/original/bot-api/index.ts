...
const APT_DISPLAY: Record<string, string> = {
  piral_1: "Оазис 1",
  piral_2: "Оазис 2",
  grande: "Гранде",
  salvador: "Сальвадор",
...
const APT_FEE: Record<string, number> = {
  piral_1: 35,
  piral_2: 35,
  grande: 70,
  salvador: 35,
...
const DIRTY_LINEN_LOCATION: Record<string, string> = {
...
const CLEANER_NAME_ALIASES: Record<string, string> = {
  Helga: "Ольга",
  Vika: "Вика",
  Viktoria: "Вика",
  Viktoriia: "Вика",
...
const CLEANER_LOOKUP_CANDIDATES: Record<string, string[]> = {
  "Ольга": ["Ольга", "Helga"],
...
function getTelegramToken(): string | null {
  return Deno.env.get("TELEGRAM_LINEN_BOT_TOKEN") ?? Deno.env.get("TELEGRAM_BOT_TOKEN") ?? null;
}
...
async function sendTg(token: string, chatId: string, text: string, parseMode = "Markdown", replyMarkup?: any) {
  try {
    const body: any = { chat_id: String(chatId), text, parse_mode: parseMode };
...
async function notifyCleaner(chatId: string, text: string) {
  const token = getTelegramToken();
...
function normalizeCleanerName(name: string | null | undefined): string | null {
  if (!name) return null;
  const trimmed = String(name).trim();
  return CLEANER_NAME_ALIASES[trimmed] ?? trimmed;
}

function getCleanerLookupCandidates(name: string | null | undefined): string[] {
  const normalized = normalizeCleanerName(name);
  if (!normalized) return [];
  return Array.from(new Set([normalized, ...(CLEANER_LOOKUP_CANDIDATES[normalized] ?? [])]));
}

function normalizeTelegramId(value: string | null | undefined): string | null {
...
function pickBestAssignment(rows: any[]): any | null {
...
async function lookupCleanerChatId(supabase: any, cleanerName: string | null | undefined): Promise<string | null> {
  ...
  const cleanerHit = (data ?? []).find((row: any) => normalizeTelegramId(row?.telegram_id));
  ...
  const assignmentHit = (assignmentHits ?? []).find((row: any) => normalizeTelegramId(row?.cleaner_telegram_id));
  ...
  const messageHit = (messageHits ?? []).find((row: any) => normalizeTelegramId(row?.chat_id));
  return normalizeTelegramId(messageHit?.chat_id);
}

async function getEquivalentAssignments(supabase: any, assignment: any): Promise<any[]> {
  ...
  try {
    switch (action) {
      ...
      case "confirm_movement": {
        ...
        const fixLocation = (loc: string | null): string | null => {
          if (!loc) return null;
      ...
      case "get_schedule": {
        ...
        const scheduleRows = (schedules ?? []).map((s: any) => {
          ...
          const matched = cleanedAssignments.filter((a: any) =>
        ...
        const manualRows = cleanedAssignments
          .filter((assignment: any) => !matchedAssignmentIds.has(assignment.id))
          .map((assignment: any) => ({
            id: assignment.id,
            schedule_id: null,
        ...
        const sortedData = suppressGrandeOverlaps([...scheduleRows, ...manualRows]).sort((a: any, b: any) => {
        ...
        const aptKw: Record<string, string[]> = {
        ...
        const enrichedSchedule = sortedData.map((slot: any) => {
      ...
      case "assign_cleaner_by_irina": {
        ...
        if (existingAssignment) {
          ...
          const targetIds = Array.from(new Set((relatedAssignments.length > 0 ? relatedAssignments : [existingAssignment]).map((row: any) => row.id)));
      ...
      case "get_cleaners": {
        try {
          ...
          const result = (data ?? []).map((c: any) => ({ name: c.name, telegram_id: c.telegram_id }));
      ...
      case "get_my_assignments": {
        ...
        return json({
          ...
          data: dedupeAssignments(data ?? [])
            .sort((a: any, b: any) => String(b.cleaning_date ?? "").localeCompare(String(a.cleaning_date ?? "")))
            .slice(0, 20),
        });
      }
      ...
      case "replace_cleaner": {
        ...
        const targetIds = Array.from(new Set((relatedAssignments.length > 0 ? relatedAssignments : [assignment]).map((row: any) => row.id)));
      ...
      case "get_bookings_with_spa": {
        ...
        const filtered = (data ?? []).filter((row: any) => {
          ...
          if (Array.isArray(tasks)) {
            return tasks.some((t: any) => (t.key === "spa" || t.key === "crib") && t.enabled === true);
          }
      ...
      case "get_bookings_by_period": {
        ...
        const aptKeywords: Record<string, string[]> = {
      ...
      case "get_new_bookings_to_notify": {
        ...
        // Atomically mark as notified and return updated rows to prevent race conditions
      ...
      case "assign_booking_tasks": {
        ...
        const updatePayload: any = { tasks_assigned: true };
        ...
        if (Array.isArray(tasks)) {
          const guestsTask = tasks.find((t: any) => t.key === "guests" && t.value != null);
          if (guestsTask) {
        ...
        if (gap_days !== undefined && gap_days <= 2 && data) {
          ...
          if (Array.isArray(tasks)) {
            taskDescriptions = tasks
              .filter((t: any) => t.enabled === true && t.key !== "guests")
      ...
      case "update_booking_task": {
        ...
        (currentTasks as any)[task_key] = value;
      ...
      case "calculate_laundry_cost": {
        ...
        const free_delivery = (items as any).sheet_set >= 15;
      ...
      case "get_albert_balance": {
        ...
        const itemTypeToKey: Record<string, string> = {
          sheets: "sheet",
      ...
      case "assign_cleaner": {
        ...
        if (existing && existing.length > 0) {
          const taken = existing.find((a: any) => a.cleaner_name && normalizeCleanerName(a.cleaner_name) !== normalizeCleanerName(cName));
          if (taken) {
      ...
      case "delete_slot": {
        ...
        const { data: cancelled, error: cancelErr } = await supabase
          .from("cleaning_assignments")
          .update({ status: "cancelled", cleaner_name: null, cleaner_telegram_id: null })
          .eq("schedule_id", slot_id)
      ...
      case "create_albert_visit": {
        ...
        const balance_after: Record<string, number> = {};
        ...
        const dirty_remaining: Record<string, Record<string, number>> = {};
        for (const loc of ["dirty_linen_piral", "dirty_linen_salvador"]) {
          ...
          const locBalance: Record<string, number> = {};
      ...
      case "get_albert_visits": {
        ...
        const result = (data ?? []).map((v: any) => {
          const deliveredCount = Object.values(v.delivered_items ?? {}).reduce((s: number, q: any) => s + Number(q), 0);
          const pickedCount = Object.values(v.picked_items ?? {}).reduce((s: number, q: any) => s + Number(q), 0);
          return {
            id: v.id,
      ...
      case "get_financial_balance": {
        ...
        const total_invoiced = (invoices ?? []).reduce((s: number, i: any) => s + (Number(i.invoice_amount) || 0), 0);
        ...
        const total_paid = (payments ?? []).reduce((s: number, p: any) => s + (Number(p.amount) || 0), 0);
        ...
        const total_factual = (visits ?? []).reduce((s: number, v: any) => s + (Number(v.delivered_cost) || 0), 0);
      ...
      case "get_transactions_by_period": {
        ...
        for (const tbl of tables) {
          ...
          if (error) throw error;
          allTx = allTx.concat((data ?? []).map((r: any) => ({ ...r, _source: tbl })));
        }

        if (filterCat) allTx = allTx.filter((t: any) => (t.category || t.description || "").toLowerCase().includes(filterCat.toLowerCase()));

        const totalIncome = allTx.filter((t: any) => t.transaction_type === "income").reduce((s: number, t: any) => s + Number(t.amount), 0);
        const totalExpense = allTx.filter((t: any) => t.transaction_type === "expense").reduce((s: number, t: any) => s + Number(t.amount), 0);
      ...
      case "get_cleaner_stats": {
        ...
        const rows = data ?? [];
        const totalPay = rows.reduce((s: number, r: any) => s + (Number(r.payment_amount) || 35), 0);
        const today = new Date().toISOString().split("T")[0];
        const past = rows.filter((r: any) => r.cleaning_date <= today);
        const future = rows.filter((r: any) => r.cleaning_date > today);
      ...
      case "get_laundry_cost_by_period": {
        ...
        const totalCost = (data ?? []).reduce((s: number, r: any) => s + (Number(r.total_laundry_cost) || 0), 0);
        const totalItems = (data ?? []).reduce((s: number, r: any) => s + (Number(r.quantity) || 0), 0);
      ...
      case "get_transactions_by_category": {
        ...
        for (const tbl of ["emma_transactions", "main_transactions"]) {
          const { data, error } = await supabase.from(tbl).select("*").gte("transaction_date", date_from).lte("transaction_date", date_to + "T23:59:59");
          if (error) throw error;
          all = all.concat((data ?? []).filter((t: any) => (t.category || t.description || "").toLowerCase().includes(cat.toLowerCase())));
        }

        return json({ success: true, data: all, total: all.reduce((s: number, t: any) => s + Number(t.amount), 0) });
      }
      ...
      case "get_income_sources": {
        ...
        return json({ success: true, data: bySource, total: all.reduce((s: number, t: any) => s + Number(t.amount), 0) });
      }
      ...
      case "get_cleaner_payment_log": {
        ...
        return json({
          success: true, data: (data ?? []).map((r: any) => ({
          ...
          total: (data ?? []).reduce((s: number, r: any) => s + (Number(r.payment_amount) || 35), 0),
        });
      }
      ...
      case "update_task_status": {
        ...
        const statusMap: Record<string, string> = { done: "completed", cancelled: "cancelled", active: "active" };
      ...
      case "get_bookings_summary": {
        ...
        const { data: incomes } = await supabase
          ...
          .gte("transaction_date", date_from)
          .lte("transaction_date", date_to + "T23:59:59");
        const totalIncome = (incomes ?? []).reduce((s: number, t: any) => s + Number(t.amount), 0);
      ...
      case "get_apartment_revenue": {
        ...
        const result: Record<string, number> = { piral_1: 0, piral_2: 0, grande: 0, salvador: 0 };
        let unmatched = 0;

        for (const t of [...(emmaData ?? []), ...(mainData ?? [])]) {
          const text = `${t.description || ""} ${t.location || ""} ${(t as any).counterparty || ""}`.toLowerCase();
          let matched = false;
      ...
      case "get_bookings_without_income": {
        ...
        const aptKwMap: Record<string, string[]> = {
          piral_1: ["оазис 1", "oasis 1", "oasis1", "piral_1"],
          piral_2: ["оазис 2", "oasis 2", "oasis2", "piral_2"],
          grande: ["гранде", "grande", "oasis_grande"],
          salvador: ["сальвадор", "salvador"],
        ...
        const withoutIncome = (bookings ?? []).filter((b: any) => {
          const keywords = aptKwMap[b.apartment] || [];
          if (keywords.length === 0) return true;
          const hasMatch = (incomes ?? []).some((t: any) => {
            const text = `${t.description || ""} ${t.location || ""}`.toLowerCase();
            return keywords.some((kw: string) => text.includes(kw));
          });
          return !hasMatch;
        ...
        const bookingIds = withoutIncome.map((b: any) => b.id);
        ...
        const result = withoutIncome.map((b: any) => ({
          ...b,

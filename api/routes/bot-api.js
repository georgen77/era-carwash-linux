...
function getCleanerLookupCandidates(name) {
  ...
  return Array.from(/* @__PURE__ */ new Set([normalized, ...CLEANER_LOOKUP_CANDIDATES[normalized] ?? []]));
}
...
function assignmentPriority(row) {
  let score = 0;
  if (normalizeCleanerName(row?.cleaner_name)) score += 1e3;
  if (normalizeTelegramId(row?.cleaner_telegram_id)) score += 100;
  if (row?.status && row.status !== "cancelled") score += 10;
  if (row?.registered_at) score += new Date(row.registered_at).getTime() / 1e15;
  return score;
}
...
async function lookupCleanerChatId(supabase, cleanerName) {
  ...
  const { data, error } = await supabase.from("cleaners").select("name, telegram_id").in("name", candidates).eq("is_active", true).order("name", { ascending: true });
  ...
  const { data: assignmentHits, error: assignmentError } = await supabase.from("cleaning_assignments").select("cleaner_name, cleaner_telegram_id, registered_at, cleaning_date").in("cleaner_name", candidates).not("cleaner_telegram_id", "is", null).order("registered_at", { ascending: false }).order("cleaning_date", { ascending: false }).limit(20);
  ...
  const { data: messageHits, error: messageError } = await supabase.from("telegram_messages").select("chat_id, user_first_name, created_at").in("user_first_name", candidates).order("created_at", { ascending: false }).limit(20);
...
async function getEquivalentAssignments(supabase, assignment) {
  ...
  if (assignment.schedule_id) {
    const { data: data2, error: error2 } = await supabase.from("cleaning_assignments").select("*").or(`schedule_id.eq.${assignment.schedule_id},and(apartment.eq.${assignment.apartment},cleaning_date.eq.${assignment.cleaning_date})`).neq("status", "cancelled");
    if (error2) throw error2;
    return (data2 ?? []).map(normalizeAssignment);
  }
  const { data, error } = await supabase.from("cleaning_assignments").select("*").eq("apartment", assignment.apartment).eq("cleaning_date", assignment.cleaning_date).neq("status", "cancelled");
  if (error) throw error;
  return (data ?? []).map(normalizeAssignment);
}
async function findAssignmentRecord(supabase, refs) {
  const directId = refs.assignment_id ?? refs.id ?? null;
  const scheduleRef = refs.schedule_id ?? refs.id ?? null;
  if (directId) {
    const { data, error } = await supabase.from("cleaning_assignments").select("*").eq("id", directId).neq("status", "cancelled").maybeSingle();
    if (error) throw error;
    if (data) return data;
  }
  if (!scheduleRef) return null;
  const { data: bySchedule, error: scheduleError } = await supabase.from("cleaning_assignments").select("*").eq("schedule_id", scheduleRef).neq("status", "cancelled");
  if (scheduleError) throw scheduleError;
  const bestBySchedule = pickBestAssignment(bySchedule ?? []);
  if (bestBySchedule) return bestBySchedule;
  const { data: scheduleRow, error: scheduleLookupError } = await supabase.from("cleaning_schedule").select("apartment, cleaning_date, checkout_date").eq("id", scheduleRef).maybeSingle();
  if (scheduleLookupError) throw scheduleLookupError;
  if (scheduleRow) {
    const effectiveDate = scheduleRow.cleaning_date ?? scheduleRow.checkout_date;
    if (effectiveDate) {
      const { data: byApartmentDate, error: apartmentDateError } = await supabase.from("cleaning_assignments").select("*").eq("apartment", scheduleRow.apartment).eq("cleaning_date", effectiveDate).neq("status", "cancelled");
  ...
  if (!directId) {
    const { data: byId, error: idError } = await supabase.from("cleaning_assignments").select("*").eq("id", scheduleRef).neq("status", "cancelled");
...
router.post("/bot-api", async (req, res) => {
  ...
  try {
    switch (action) {
      ...
      case "get_pending": {
        const { data, error } = await supabase.from("pending_movements").select("*").eq("confirmed", false).order("created_at", { ascending: false });
        if (error) throw error;
      ...
      case "confirm_movement": {
        ...
        const { data: pending, error: fetchErr } = await supabase.from("pending_movements").select("*").eq("id", id).single();
        if (fetchErr || !pending) return res.status(404).json({ error: "Pending movement not found" });
        if (pending.confirmed === true) {
          return res.json({ success: false, already_confirmed: true });
        }
        ...
        for (const item of items) {
          const { data: mov, error: insErr } = await supabase.from("movements").insert({
        ...
        await supabase.from("pending_movements").update({ confirmed: true }).eq("id", id);
        return res.json({ success: true, inserted_count: inserted.length, data: inserted });
      }
      // ─── 3. REJECT MOVEMENT ─────────────────────────────────────────────
      case "reject_movement": {
        const { id } = body;
        if (!id) return res.status(400).json({ error: "Missing id" });
        const { error } = await supabase.from("pending_movements").delete().eq("id", id);
        if (error) throw error;
      ...
      case "save_pending": {
        ...
        const { data, error } = await supabase.from("pending_movements").insert({
          items: items ?? null,
      ...
      case "get_schedule": {
        ...
        const in60 = new Date(Date.now() + 60 * 24 * 60 * 60 * 1e3).toISOString().split("T")[0];
        const { data: schedules, error } = await supabase.from("cleaning_schedule").select("*").gte("checkout_date", today).lte("checkout_date", in60).order("checkout_date", { ascending: true });
        if (error) throw error;
        const { data: allAssignments, error: assignmentsError } = await supabase.from("cleaning_assignments").select("id, schedule_id, cleaner_name, cleaner_telegram_id, status, apartment, cleaning_date, next_guests, payment_amount, registered_at").gte("cleaning_date", today).lte("cleaning_date", in60).neq("status", "cancelled");
        if (assignmentsError) throw assignmentsError;
        const cleanedAssignments = dedupeAssignments(allAssignments ?? []);
        const matchedAssignmentIds = /* @__PURE__ */ new Set();
        const scheduleRows = (schedules ?? []).map((s) => {
          const effectiveDate = getEffectiveDate(s);
          const matched = cleanedAssignments.filter(
            (a) => a.schedule_id && a.schedule_id === s.id || !a.schedule_id && a.apartment === s.apartment && a.cleaning_date === effectiveDate
          );
        ...
        const manualRows = cleanedAssignments.filter((assignment) => !matchedAssignmentIds.has(assignment.id)).map((assignment) => ({
          id: assignment.id,
          schedule_id: null,
        ...
        const { data: allIncomes } = await supabase.from("main_transactions").select("amount, description, location").eq("transaction_type", "income").gte("transaction_date", today).lte("transaction_date", in60 + "T23:59:59");
        const aptKw = {
      ...
      case "update_schedule_guests": {
        ...
        if (!schedule_id || next_guests === void 0) {
          return res.status(400).json({ error: "Missing schedule_id or next_guests" });
        }
        const { error } = await supabase.from("cleaning_schedule").update({ next_guests: Number(next_guests) }).eq("id", schedule_id);
        if (error) throw error;
        return res.json({ success: true });
      }
      // ─── UPDATE SLOT ────────────────────────────────────────────────────────
      case "update_slot": {
        ...
        const updateFields = {};
        if (next_guests !== void 0) updateFields.next_guests = Number(next_guests);
        if (special_instructions !== void 0) updateFields.special_instructions = special_instructions;
        if (Object.keys(updateFields).length === 0) {
          return res.status(400).json({ error: "No fields to update" });
        }
        console.log("[update_slot] updating cleaning_schedule id=", slot_id, "fields=", JSON.stringify(updateFields));
        const { data: scheduleData, error: scheduleErr } = await supabase.from("cleaning_schedule").update(updateFields).eq("id", slot_id).select();
        if (scheduleErr) {
        ...
        if (scheduleData && scheduleData.length > 0) {
          return res.json({ success: true, data: scheduleData[0] });
        }
        console.log("[update_slot] not found in schedule, checking assignments...");
        const { data: assignment, error: aErr } = await supabase.from("cleaning_assignments").select("schedule_id").eq("id", slot_id).maybeSingle();
        if (aErr) {
        ...
        if (assignment?.schedule_id) {
          ...
          const { data: updated, error: updateErr } = await supabase.from("cleaning_schedule").update(updateFields).eq("id", assignment.schedule_id).select().single();
          if (updateErr) {
        ...
        return res.json({ error: "Slot not found", slot_id }, 404);
      }
      ...
      case "assign_cleaner_by_irina": {
        ...
        if (!slotRef && !assignment_id || !normalizedCleanerName) {
          return res.status(400).json({ error: "Missing schedule_id/assignment_id/id or cleaner_name" });
        }
        const existingAssignment = await findAssignmentRecord(supabase, { schedule_id: slotRef, assignment_id, id });
        if (existingAssignment) {
          const apartment2 = apartment_override ?? existingAssignment.apartment;
          const cleaning_date2 = existingAssignment.cleaning_date;
          const aptName2 = APT_DISPLAY[apartment2] ?? apartment2;
          ...
          const updatePayload = {
            ...
            cleaning_date: cleaning_date2,
            status: existingAssignment.status ?? "assigned"
          };
          const { error: updErr } = targetIds.length > 1 ? await supabase.from("cleaning_assignments").update(updatePayload).in("id", targetIds) : await supabase.from("cleaning_assignments").update(updatePayload).eq("id", existingAssignment.id);
          ...
          if (normalizedCleanerChatId) {
            await notifyCleaner(
              normalizedCleanerChatId,
              `\u{1F4C5} \u0414\u043E\u0431\u0440\u044B\u0439 \u0434\u0435\u043D\u044C, ${firstName(normalizedCleanerName)}! \u0412\u0430\u043C \u043D\u0430\u0437\u043D\u0430\u0447\u0435\u043D\u0430 \u0443\u0431\u043E\u0440\u043A\u0430:
\u{1F3E0} ${aptName2}
\u{1F4C6} \u0412\u044B\u0435\u0437\u0434: ${fmtDate(cleaning_date2)}
\u{1F465} ${existingAssignment.next_guests ?? 4} \u0433\u043E\u0441\u0442\u0435\u0439`
            );
          }
          await notifyAdmins(
            `\u{1F4CB} *\u041D\u0430\u0437\u043D\u0430\u0447\u0435\u043D\u0438\u0435 \u0441\u043C\u0435\u043D\u044B (\u0418\u0440\u0438\u043D\u0430)*
\u{1F464} ${normalizedCleanerName}
\u{1F3E0} ${aptName2}
\u{1F4C6} ${fmtDate(cleaning_date2)}`
          );
          return res.json({ success: true, apartment: apartment2, cleaning_date: cleaning_date2, assignment: updated });
        }
        const { data: schedule, error: schedErr } = await supabase.from("cleaning_schedule").select("apartment, checkout_date, checkin_date, next_guests").eq("id", slotRef).single();
        if (schedErr || !schedule) return res.status(404).json({ error: "Schedule not found" });
        ...
        const { data: existing } = await supabase.from("cleaning_assignments").select("id").eq("schedule_id", slotRef).neq("status", "cancelled").maybeSingle();
        let assignment;
        if (existing) {
          const { data: updated, error: updErr } = await supabase.from("cleaning_assignments").update({
            cleaner_name: normalizedCleanerName,
            cleaner_telegram_id: normalizedCleanerChatId ?? null,
            apartment,
            cleaning_date
          ...
          const { data: inserted, error: insErr } = await supabase.from("cleaning_assignments").insert({
            schedule_id: slotRef,
            apartment,
            cleaning_date,
            cleaner_name: normalizedCleanerName,
        ...
        if (normalizedCleanerChatId) {
          await notifyCleaner(
            ...
            `\u{1F4C5} \u0414\u043E\u0431\u0440\u044B\u0439 \u0434\u0435\u043D\u044C, ${firstName(normalizedCleanerName)}! \u0412\u0430\u043C \u043D\u0430\u0437\u043D\u0430\u0447\u0435\u043D\u0430 \u0443\u0431\u043E\u0440\u043A\u0430:
\u{1F3E0} ${aptName}
\u{1F4C6} \u0412\u044B\u0435\u0437\u0434: ${fmtDate(cleaning_date)}
\u{1F465} ${schedule.next_guests ?? 4} \u0433\u043E\u0441\u0442\u0435\u0439`
          );
        }
        await notifyAdmins(
          `\u{1F4CB} *\u041D\u0430\u0437\u043D\u0430\u0447\u0435\u043D\u0438\u0435 \u0441\u043C\u0435\u043D\u044B (\u0418\u0440\u0438\u043D\u0430)*
\u{1F464} ${normalizedCleanerName}
\u{1F3E0} ${aptName}
\u{1F4C6} ${fmtDate(cleaning_date)}`
        );
        return res.json({ success: true, apartment, cleaning_date, assignment });
      }
      // ─── 6. GET CLEANERS ─────────────────────────────────────────────────
      case "get_cleaners": {
        try {
          console.log("[get_cleaners] called, fetching active cleaners...");
          const { data, error } = await supabase.from("cleaners").select("name, telegram_id").eq("is_active", true).order("name", { ascending: true });
          if (error) {
      ...
      case "sync_ical": {
        ...
        if (!resp.ok) {
          return res.json({ success: false, error: result?.error ?? "sync-ical failed" }, resp.status);
        }
        return res.json({
          success: true,
          added: result.new ?? 0,
      ...
      case "get_my_assignments": {
        const { chat_id } = body;
        if (!chat_id) return res.status(400).json({ error: "Missing chat_id" });
        const { data, error } = await supabase.from("cleaning_assignments").select("*").eq("cleaner_telegram_id", String(chat_id)).order("cleaning_date", { ascending: false });
        if (error) throw error;
        return res.json({
          success: true,
          data: dedupeAssignments(data ?? []).sort((a, b) => String(b.cleaning_date ?? "").localeCompare(String(a.cleaning_date ?? ""))).slice(0, 20)
        });
      }
      // ─── 8. SIGNUP CLEANING ──────────────────────────────────────────────
      case "signup_cleaning": {
        ...
        if (!slotRef || !chat_id || !cleaner_name) {
          return res.json({ error: "Missing schedule_id/id, chat_id, or cleaner_name" }, 400);
        }
        const normalizedCleanerName = normalizeCleanerName(cleaner_name);
        ...
        if (existing) {
          if (existing.cleaner_name) {
            return res.json({ success: false, conflict: true, taken_by: existing.cleaner_name });
          }
          const { data: updated, error: updateErr } = await supabase.from("cleaning_assignments").update({
            cleaner_name: normalizedCleanerName,
            cleaner_telegram_id: normalizedChatId,
            status: "assigned",
            registered_at: existing.registered_at ?? (/* @__PURE__ */ new Date()).toISOString()
          }).eq("id", existing.id).select().single();
          if (updateErr) throw updateErr;
          const aptName2 = APT_DISPLAY[existing.apartment] ?? existing.apartment;
          await notifyCleaner(
            String(chat_id),
            `\u2705 \u0414\u043E\u0431\u0440\u044B\u0439 \u0434\u0435\u043D\u044C, ${firstName(normalizedCleanerName)}! \u0412\u044B \u0443\u0441\u043F\u0435\u0448\u043D\u043E \u0437\u0430\u043F\u0438\u0441\u0430\u043B\u0438\u0441\u044C \u043D\u0430 \u0443\u0431\u043E\u0440\u043A\u0443:
\u{1F3E0} ${aptName2}
\u{1F4C6} \u0412\u044B\u0435\u0437\u0434: ${fmtDate(existing.cleaning_date)}
\u{1F465} ${existing.next_guests ?? 4} \u0433\u043E\u0441\u0442\u0435\u0439`
          );
          await notifyAdmins(
            `\u270B *\u0417\u0430\u043F\u0438\u0441\u044C \u043D\u0430 \u0443\u0431\u043E\u0440\u043A\u0443*
\u{1F464} ${normalizedCleanerName} \u0437\u0430\u043F\u0438\u0441\u0430\u043B\u0430\u0441\u044C
\u{1F3E0} ${aptName2}
\u{1F4C6} ${fmtDate(existing.cleaning_date)}`
          );
          return res.json({
            success: true,
        ...
        const { data: schedule, error: schedErr } = await supabase.from("cleaning_schedule").select("apartment, checkout_date, checkin_date, next_guests").eq("id", slotRef).single();
        if (schedErr || !schedule) return res.status(404).json({ error: "Schedule not found" });
        const { data: assignment, error: insErr } = await supabase.from("cleaning_assignments").insert({
          schedule_id: slotRef,
        ...
        await notifyCleaner(
          ...
          `\u2705 \u0414\u043E\u0431\u0440\u044B\u0439 \u0434\u0435\u043D\u044C, ${firstName(normalizedCleanerName)}! \u0412\u044B \u0443\u0441\u043F\u0435\u0448\u043D\u043E \u0437\u0430\u043F\u0438\u0441\u0430\u043B\u0438\u0441\u044C \u043D\u0430 \u0443\u0431\u043E\u0440\u043A\u0443:
\u{1F3E0} ${aptName}
\u{1F4C6} \u0412\u044B\u0435\u0437\u0434: ${fmtDate(schedule.checkout_date)}
${schedule.checkin_date ? `\u{1F4C6} \u0417\u0430\u0435\u0437\u0434: ${fmtDate(schedule.checkin_date)}
` : ""}\u{1F465} ${schedule.next_guests ?? 4} \u0433\u043E\u0441\u0442\u0435\u0439`
        );
        await notifyAdmins(
          `\u270B *\u0417\u0430\u043F\u0438\u0441\u044C \u043D\u0430 \u0443\u0431\u043E\u0440\u043A\u0443*
\u{1F464} ${normalizedCleanerName} \u0437\u0430\u043F\u0438\u0441\u0430\u043B\u0430\u0441\u044C
\u{1F3E0} ${aptName}
\u{1F4C6} ${fmtDate(schedule.checkout_date)}`
        );
        return res.json({
          success: true,
          apartment: schedule.apartment,
      ...
      case "complete_cleaning": {
        ...
        const { data: assignment, error: fetchErr } = await supabase.from("cleaning_assignments").select("*").eq("id", assignment_id).single();
        ...
        await supabase.from("cleaning_assignments").update({ status: "completed", finished_at: (/* @__PURE__ */ new Date()).toISOString(), payment_amount: fee }).eq("id", assignment_id);
        const { data: systemUser } = await supabase.from("cleaning_users").select("id").in("role", ["admin", "coordinator"]).eq("is_active", true).limit(1).single();
        const createdBy = systemUser?.id ?? "00000000-0000-0000-0000-000000000000";
        const { data: tx, error: txErr } = await supabase.from("emma_transactions").insert({
        ...
        await supabase.from("cleaning_assignments").update({ payment_transaction_id: tx.id }).eq("id", assignment_id);
        ...
        const { error: linenErr } = await supabase.from("movements").insert(linenInserts);
        if (linenErr) console.error("[complete_cleaning] linen insert error:", linenErr);
        if (cleaner_telegram_id) {
          await notifyCleaner(
            cleaner_telegram_id,
            `\u{1F389} ${firstName(cleaner_name)}, \u0443\u0431\u043E\u0440\u043A\u0430 \u0437\u0430\u0432\u0435\u0440\u0448\u0435\u043D\u0430!
\u{1F3E0} ${aptDisplay}
\u{1F4C6} ${fmtDate(cleaning_date)}
\u{1F4B0} \u041A \u0432\u044B\u043F\u043B\u0430\u0442\u0435: ${fee}\u20AC`
          );
        }
        await notifyAdmins(
          `\u2705 *\u0423\u0431\u043E\u0440\u043A\u0430 \u0437\u0430\u0432\u0435\u0440\u0448\u0435\u043D\u0430*
\u{1F464} ${cleaner_name}
\u{1F3E0} ${aptDisplay}
\u{1F4C6} ${fmtDate(cleaning_date)}
\u{1F4B0} ${fee}\u20AC`
        );
      ...
      case "confirm_payment": {
        ...
        await supabase.from("cleaning_assignments").update({
          status: "paid",
          confirmed_at: now,
        ...
        if (assignment.cleaner_telegram_id) {
          await notifyCleaner(
            assignment.cleaner_telegram_id,
            `\u{1F4B0} ${firstName(assignment.cleaner_name)}, \u043E\u043F\u043B\u0430\u0442\u0430 \u043F\u043E\u0434\u0442\u0432\u0435\u0440\u0436\u0434\u0435\u043D\u0430!
\u{1F3E0} ${aptDisplay}
\u{1F4C6} ${fmtDate(assignment.cleaning_date)}
\u{1F4B5} ${assignment.payment_amount ?? 35}\u20AC`
          );
        }
        await notifyAdmins(
          `\u{1F4B0} *\u041E\u043F\u043B\u0430\u0442\u0430 \u043F\u043E\u0434\u0442\u0432\u0435\u0440\u0436\u0434\u0435\u043D\u0430*
\u{1F464} ${assignment.cleaner_name}
\u{1F3E0} ${aptDisplay}
\u{1F4C6} ${fmtDate(assignment.cleaning_date)}
\u{1F4B5} ${assignment.payment_amount ?? 35}\u20AC`
        );
      ...
      case "replace_cleaner": {
        ...
        if (!schedule_id && !assignment_id && !id || !new_cleaner_name) {
          return res.status(400).json({ error: "Missing schedule_id/slot_id/assignment_id/id or new_cleaner_name" });
        }
        ...
        if (!assignment) {
          ...
          const { data: newAssignment, error: newErr } = await supabase.from("cleaning_assignments").insert({
            schedule_id: sid,
            apartment: apt,
          ...
          const aptName2 = APT_DISPLAY[apt] ?? apt;
          if (normalizedNewCleanerChatId) {
            await notifyCleaner(
              normalizedNewCleanerChatId,
              `\u{1F4C5} \u0414\u043E\u0431\u0440\u044B\u0439 \u0434\u0435\u043D\u044C, ${firstName(normalizedNewCleanerName)}! \u0412\u0430\u043C \u043D\u0430\u0437\u043D\u0430\u0447\u0435\u043D\u0430 \u0443\u0431\u043E\u0440\u043A\u0430:
\u{1F3E0} ${aptName2}
\u{1F4C6} ${fmtDate(effectiveDate)}`
            );
          }
          await notifyAdmins(`\u2705 \u041D\u0430\u0437\u043D\u0430\u0447\u0435\u043D\u0430 \u0443\u0431\u043E\u0440\u043A\u0430: ${aptName2} ${fmtDate(effectiveDate)} \u2014 ${normalizedNewCleanerName}`);
          return res.json({ success: true, created: true, data: normalizeAssignment(newAssignment) });
        }
        ...
        const { error: updateErr } = targetIds.length > 1 ? await supabase.from("cleaning_assignments").update(updatePayload).in("id", targetIds) : await supabase.from("cleaning_assignments").update(updatePayload).eq("id", assignment.id);
        if (updateErr) throw updateErr;
        const aptName = APT_DISPLAY[assignment.apartment] ?? assignment.apartment;
        if (previousCleanerChatId) {
          await notifyCleaner(
            previousCleanerChatId,
...
\u{1F4C6} ${fmtDate(assignment.cleaning_date)}`
        ...
        await notifyAdmins(
          `\u{1F504} *\u0417\u0430\u043C\u0435\u043D\u0430 \u0443\u0431\u043E\u0440\u0449\u0438\u0446\u044B*
\u{1F3E0} ${aptName} \xB7 ${fmtDate(assignment.cleaning_date)}
\u274C ${previousCleanerName ?? "\u2014"} \u2192 \u2705 ${normalizedNewCleanerName}`
      ...
      case "remove_cleaner": {
        ...
        if (!schedule_id && !assignment_id && !id) {
          return res.json({ error: "Missing schedule_id, assignment_id, or id" }, 400);
        }
        const assignment = await findAssignmentRecord(supabase, { schedule_id, assignment_id, id });
        ...
        const { error: delErr } = targetIds.length > 1 ? await supabase.from("cleaning_assignments").delete().in("id", targetIds) : await supabase.from("cleaning_assignments").delete().eq("id", assignment.id);
        if (delErr) throw delErr;
        const aptName = APT_DISPLAY[assignment.apartment] ?? assignment.apartment;
        if (normalizedRemovedCleanerChatId) {
          await notifyCleaner(
            normalizedRemovedCleanerChatId,
...
\u{1F4C6} ${fmtDate(assignment.cleaning_date)}`
        ...
        await notifyAdmins(
          `\u{1F5D1} *\u0423\u0434\u0430\u043B\u0435\u043D\u0438\u0435 \u0443\u0431\u043E\u0440\u0449\u0438\u0446\u044B \u0438\u0437 \u0441\u043C\u0435\u043D\u044B*
\u{1F464} ${normalizedRemovedCleanerName ?? "\u2014"}
\u{1F3E0} ${aptName}
\u{1F4C6} ${fmtDate(assignment.cleaning_date)}`
        );
        return res.json({
      ...
      case "get_pending_by_id": {
        const { id } = body;
        if (!id) return res.status(400).json({ error: "Missing id" });
        const { data, error } = await supabase.from("pending_movements").select("*").eq("id", id).single();
        if (error) return res.status(404).json({ error: "Not found" });
        return res.json({ success: true, data });
      }
      // ─── 13. AUTO CONFIRM PENDING ────────────────────────────────────────
      case "auto_confirm_pending": {
        const { data: stale, error: fetchErr } = await supabase.from("pending_movements").select("*").eq("confirmed", false).lt("created_at", new Date(Date.now() - 24 * 60 * 60 * 1e3).toISOString());
        if (fetchErr) throw fetchErr;
        if (!stale || stale.length === 0) return res.json({ success: true, auto_confirmed: 0 });
        let confirmed = 0;
        for (const pending of stale) {
          ...
          for (const item of items) {
            ...
            if (insErr) {
              allOk = false;
              console.error("[auto_confirm] insert error:", insErr);
            }
          }
          if (allOk) {
        ...
        return res.json({ success: true, auto_confirmed: confirmed });
      }
      // ─── NOTIFY CLEANER ──────────────────────────────────────────────────
      case "notify_cleaner": {
        const { cleaner_chat_id, apartment_name, cleaning_date, message } = body;
        if (!cleaner_chat_id) return res.status(400).json({ error: "Missing cleaner_chat_id" });
        const telegramToken = getTelegramToken();
        if (!telegramToken) return res.status(500).json({ error: "No Telegram token configured" });
        const text = message ?? `\u{1F4C5} \u0412\u0430\u0448\u0430 \u0441\u043C\u0435\u043D\u0430!
\u{1F3E0} ${apartment_name ?? ""}
\u{1F4C6} ${cleaning_date ?? ""}`;
        ...
        if (!tgResp.ok) {
          return res.json({ error: "Telegram send failed", details: tgData }, 502);
        }
      ...
      case "notify_new_booking": {
        ...
        if (!schedule_id || !apartment) return res.status(400).json({ error: "Missing schedule_id or apartment" });
        const telegramToken = getTelegramToken();
        if (!telegramToken) return res.status(500).json({ error: "No Telegram token configured" });
        const aptName = APT_DISPLAY[apartment] ?? apartment;
        ...
        const { data: cleaners } = await supabase.from("cleaners").select("name, telegram_id").eq("is_active", true).not("telegram_id", "is", null);
        let notified = 0;
...
\u{1F4C6} \u0417\u0430\u0435\u0437\u0434: ${checkinFmt || "\u2014"}
\u{1F4C6} \u0412\u044B\u0435\u0437\u0434: ${checkoutFmt}
\u{1F465} ${guests} \u0433\u043E\u0441\u0442\u0435\u0439
...
\u041F\u0440\u0438 \u0436\u0435\u043B\u0430\u043D\u0438\u0438 \u0438 \u0432\u043E\u0437\u043C\u043E\u0436\u043D\u043E\u0441\u0442\u0438 \u0437\u0430\u043F\u0438\u0441\u0430\u0442\u044C\u0441\u044F \u043D\u0430 \u044D\u0442\u0443 \u0441\u043C\u0435\u043D\u0443 \u043E\u0442\u043C\u0435\u0442\u044C\u0442\u0435 \u044D\u0442\u043E \u0432 \u0440\u0430\u0441\u043F\u0438\u0441\u0430\u043D\u0438\u0438, \u043D\u0430\u0436\u0430\u0432 \u043D\u0430 \u043A\u043D\u043E\u043F\u043A\u0443 \u043D\u0438\u0436\u0435.`;
        ...
        const adminText = `\u{1F3E0} *\u041D\u043E\u0432\u043E\u0435 \u0431\u0440\u043E\u043D\u0438\u0440\u043E\u0432\u0430\u043D\u0438\u0435*
...
\u{1F465} ${guests} \u0433\u043E\u0441\u0442\u0435\u0439`;
      ...
      case "start_cleaning": {
        const { assignment_id } = body;
        if (!assignment_id) return res.status(400).json({ error: "Missing assignment_id" });
        const { data: assignment, error: fetchErr } = await supabase.from("cleaning_assignments").select("*").eq("id", assignment_id).maybeSingle();
        if (fetchErr || !assignment) {
          return res.status(8).json({ error: `Assignment not found (ID: ${assignment_id?.slice(0)}\u2026)` }, 404);
        }
        ...
        const now = (/* @__PURE__ */ new Date()).toISOString();
        const { error: updErr } = await supabase.from("cleaning_assignments").update({ status: "started", started_at: now }).eq("id", assignment_id);
        if (updErr) throw updErr;
        const aptDisplay = APT_DISPLAY[assignment.apartment] ?? assignment.apartment;
        await notifyAdmins(
          `\u{1F9F9} *\u0423\u0431\u043E\u0440\u043A\u0430 \u043D\u0430\u0447\u0430\u0442\u0430*
\u{1F464} ${assignment.cleaner_name}
\u{1F3E0} ${aptDisplay}
\u{1F4C6} ${fmtDate(assignment.cleaning_date)}`
        );
      ...
      case "get_bookings_with_spa": {
        const today = (/* @__PURE__ */ new Date()).toISOString().split("T")[0];
        const { data, error } = await supabase.from("cleaning_schedule").select("*").gte("checkin_date", today).eq("tasks_assigned", true).order("checkin_date", { ascending: true });
        if (error) throw error;
      ...
      case "get_bookings_with_tasks": {
        const today = (/* @__PURE__ */ new Date()).toISOString().split("T")[0];
        const { data, error } = await supabase.from("cleaning_schedule").select("id, apartment, checkin_date, checkout_date, guests_count, tasks, source, gap_days").gte("checkin_date", today).eq("tasks_assigned", true).order("checkin_date", { ascending: true });
        if (error) throw error;
      ...
      case "get_bookings_by_period": {
        ...
        if (!date_from || !date_to) return res.status(400).json({ error: "Missing date_from or date_to" });
        const { data, error } = await supabase.from("cleaning_schedule").select("id, apartment, checkin_date, checkout_date, guests_count, source, tasks, gap_days").lt("checkin_date", date_to).gt("checkout_date", date_from).order("checkin_date", { ascending: true });
        if (error) throw error;
        const { data: incomes } = await supabase.from("main_transactions").select("amount, description, location").eq("transaction_type", "income").gte("transaction_date", date_from).lte("transaction_date", date_to + "T23:59:59");
        const aptKeywords = {
        ...
        const enriched = (data ?? []).map((booking) => {
          ...
          const matchedIncome = (incomes ?? []).filter((t) => {
            const text = `${t.description || ""} ${t.location || ""}`.toLowerCase();
            return keywords.some((kw) => text.includes(kw));
          }).reduce((s, t) => s + Number(t.amount), 0);
      ...
      case "get_cleaning_tasks_for_slot": {
        const { slot_id } = body;
        if (!slot_id) return res.status(400).json({ error: "Missing slot_id" });
        const { data: slot, error: slotErr } = await supabase.from("cleaning_schedule").select("id, apartment, checkout_date, checkin_date").eq("id", slot_id).single();
        if (slotErr || !slot) return res.status(404).json({ error: "Slot not found" });
        const { data: nextBooking, error: nextErr } = await supabase.from("cleaning_schedule").select("id, checkin_date, checkout_date, guests_count, tasks, gap_days").eq("apartment", slot.apartment).gt("checkin_date", slot.checkout_date).order("checkin_date", { ascending: true }).limit(1).maybeSingle();
        if (nextErr) throw nextErr;
        if (!nextBooking) {
          return res.json({ success: true, data: null });
        }
        const gapDays = nextBooking.checkin_date && slot.checkout_date ? Math.round((new Date(nextBooking.checkin_date).getTime() - new Date(slot.checkout_date).getTime()) / 864e5) : null;
        return res.json({
          success: true,
          data: {
            gap_days: gapDays,
      ...
      case "get_new_bookings_to_notify": {
        ...
        const { data: marked, error: markErr } = await supabase.from("cleaning_schedule").update({ notified: true }).eq("notified", false).gte("created_at", twoHoursAgo).select("id, apartment, checkin_date, checkout_date, guests_count, source, gap_days").order("checkin_date", { ascending: true });
      ...
      case "get_next_booking_for_apt": {
        ...
        if (!apartment || !checkout_date) return res.status(400).json({ error: "Missing apartment or checkout_date" });
        const { data, error } = await supabase.from("cleaning_schedule").select("id, checkin_date, checkout_date, guests_count, tasks, gap_days, source").eq("apartment", apartment).gt("checkin_date", checkout_date).order("checkin_date", { ascending: true }).limit(1).maybeSingle();
        if (error) throw error;
      ...
      case "assign_booking_tasks": {
        ...
        if (tasks !== void 0) updatePayload.tasks = tasks;
        if (gap_days !== void 0) updatePayload.gap_days = gap_days;
        if (Array.isArray(tasks)) {
        ...
        const { data, error } = await supabase.from("cleaning_schedule").update(updatePayload).eq("id", booking_id).select().single();
        if (error) throw error;
        if (gap_days !== void 0 && gap_days <= 2 && data) {
          let taskDescriptions = "";
          if (Array.isArray(tasks)) {
            taskDescriptions = tasks.filter((t) => t.enabled === true && t.key !== "guests").map((t) => t.name || t.key).join(", ");
          } else if (tasks && typeof tasks === "object") {
            taskDescriptions = Object.entries(tasks).filter(([_, v]) => v === true).map(([k]) => k).join(", ");
          }
          if (taskDescriptions) {
            await supabase.from("cleaning_schedule").update({ special_instructions: `\u0417\u0430\u0434\u0430\u043D\u0438\u044F: ${taskDescriptions}` }).eq("apartment", data.apartment).eq("checkout_date", data.checkin_date);
          }
        }
        return res.json({ success: true, data });
      }
      ...
      case "update_booking_task": {
        ...
        if (!booking_id || !task_key) return res.status(400).json({ error: "Missing booking_id or task_key" });
        const { data: booking, error: fetchErr } = await supabase.from("cleaning_schedule").select("tasks").eq("id", booking_id).single();
        if (fetchErr || !booking) return res.status(404).json({ error: "Booking not found" });
        const currentTasks = booking.tasks && typeof booking.tasks === "object" ? booking.tasks : {};
        currentTasks[task_key] = value;
        const { data, error } = await supabase.from("cleaning_schedule").update({ tasks: currentTasks }).eq("id", booking_id).select().single();
        if (error) throw error;
        return res.json({ success: true, data });
      }
      ...
      case "emma_confirmed_preparation": {
        ...
        if (removed_items && Object.keys(removed_items).length > 0) {
          const { data: booking } = await supabase.from("cleaning_schedule").select("apartment").eq("id", booking_id).single();
          if (booking) {
        ...
        return res.json({ success: true, movement_id });
      }
      // ─── LAUNDRY: GET PRICES ─────────────────────────────────────────────
      case "get_laundry_prices": {
        const { data, error } = await supabase.from("laundry_prices").select("*").eq("active", true).order("item_key", { ascending: true });
        if (error) throw error;
        return res.json({ success: true, data: data ?? [] });
      }
      ...
      case "update_laundry_price": {
        ...
        if (!item_key || price === void 0) return res.status(400).json({ error: "Missing item_key or price" });
        const { data, error } = await supabase.from("laundry_prices").update({ price: Number(price) }).eq("item_key", item_key).select().single();
        if (error) throw error;
        return res.json({ success: true, data });
      }
      ...
      case "calculate_laundry_cost": {
        const { items } = body;
        if (!items || typeof items !== "object") return res.status(400).json({ error: "Missing items object" });
        const { data: prices, error } = await supabase.from("laundry_prices").select("item_key, price").eq("active", true);
        if (error) throw error;
        const priceMap = {};
        for (const p of prices ?? []) priceMap[p.item_key] = Number(p.price);
        let subtotal = 0;
        for (const [key, qty] of Object.entries(items)) {
          if (priceMap[key] !== void 0) subtotal += priceMap[key] * Number(qty);
        }
        const vat = subtotal * 0.21;
        const total = subtotal + vat;
        const free_delivery = items.sheet_set >= 15;
        return res.json({ success: true, data: { subtotal, vat, total, free_delivery } });
      }
      // ─── LAUNDRY: GET ALBERT BALANCE ─────────────────────────────────────
      case "get_albert_balance": {
        const { data: sentData, error: sentErr } = await supabase.from("movements").select("item_type, quantity").eq("to_location", "albert_laundry");
        if (sentErr) throw sentErr;
        const { data: recvData, error: recvErr } = await supabase.from("movements").select("item_type, quantity").eq("from_location", "albert_laundry");
        ...
        const { data: prices } = await supabase.from("laundry_prices").select("item_key, price").eq("active", true);
        const priceMap = {};
        for (const p of prices ?? []) priceMap[p.item_key] = Number(p.price);
        const itemTypeToKey = {
          sheets: "sheet",
        ...
        return res.json({ success: true, data: { items: balance, calculated_cost, vat: vat2, total: calculated_cost + vat2 } });
      }
      ...
      case "get_laundry_monthly_summary": {
        ...
        const { data: movements, error } = await supabase.from("movements").select("*").or(`to_location.eq.albert_laundry,from_location.eq.albert_laundry`).gte("created_at", startDate).lt("created_at", endDate).order("created_at", { ascending: true });
        if (error) throw error;
      ...
      case "save_laundry_invoice": {
        ...
        const { data, error } = await supabase.from("laundry_invoices").insert({
          invoice_number,
          period_from,
          period_to,
      ...
      case "get_laundry_invoices": {
        const { data, error } = await supabase.from("laundry_invoices").select("*").order("created_at", { ascending: false });
        if (error) throw error;
        return res.json({ success: true, data: data ?? [] });
      }
      ...
      case "open_pending_delivery": {
        ...
        const { data, error } = await supabase.from("laundry_pending_deliveries").insert({ date, type, notes: notes ?? null, confirmed: false }).select().single();
        if (error) throw error;
        return res.json({ success: true, data });
      }
      ...
      case "confirm_pending_delivery": {
        const { id, items } = body;
        if (!id) return res.status(400).json({ error: "Missing id" });
        const { data: delivery, error: fetchErr } = await supabase.from("laundry_pending_deliveries").select("*").eq("id", id).single();
        if (fetchErr || !delivery) return res.status(404).json({ error: "Delivery not found" });
        await supabase.from("laundry_pending_deliveries").update({ confirmed: true, items: items ?? null }).eq("id", id);
        const movementIds = [];
        if (items && typeof items === "object") {
          for (const [itemType, qty] of Object.entries(items)) {
            if (Number(qty) > 0) {
              const fromLoc = delivery.type === "incoming" ? "albert_laundry" : void 0;
        ...
        return res.json({ success: true, movement_ids: movementIds });
      }
      // ─── LAUNDRY: GET UNCONFIRMED DELIVERIES ─────────────────────────────
      case "get_pending_deliveries_unconfirmed": {
        const { data, error } = await supabase.from("laundry_pending_deliveries").select("*").eq("confirmed", false).order("date", { ascending: false });
        if (error) throw error;
        return res.json({ success: true, data: data ?? [] });
      }
      // ─── LAUNDRY: UPLOAD FILE ────────────────────────────────────────────
      case "upload_laundry_file": {
        ...
        if (!file_data || !file_name) return res.status(400).json({ error: "Missing file_data or file_name" });
        ...
        const { error: uploadErr } = await supabase.storage.from("receipts").upload(path, bytes, { upsert: true });
        if (uploadErr) throw uploadErr;
        const { data: urlData } = supabase.storage.from("receipts").getPublicUrl(path);
        const file_url = urlData?.publicUrl;
        if (invoice_id) {
          const updateField = file_type === "payment" ? "payment_file_url" : "invoice_file_url";
          await supabase.from("laundry_invoices").update({ [updateField]: file_url }).eq("id", invoice_id);
        }
        return res.json({ success: true, file_url });
      }
      ...
      case "save_albert_payment": {
        ...
        const { data: systemUser } = await supabase.from("cleaning_users").select("id").in("role", ["admin", "coordinator"]).eq("is_active", true).limit(1).single();
        const createdBy = systemUser?.id ?? "00000000-0000-0000-0000-000000000000";
        const { data, error } = await supabase.from("emma_transactions").insert({
          transaction_type: "expense",
          amount: Number(amount),
          description: description ?? "\u041E\u043F\u043B\u0430\u0442\u0430 \u043F\u0440\u0430\u0447\u0435\u0447\u043D\u043E\u0439 \u0410\u043B\u044C\u0431\u0435\u0440\u0442",
          counterparty: "\u041F\u0440\u0430\u0447\u0435\u0447\u043D\u0430\u044F \u0410\u043B\u044C\u0431\u0435\u0440\u0442",
          location: "\u041F\u0440\u0430\u0447\u0435\u0447\u043D\u0430\u044F",
          transaction_date: date ?? (/* @__PURE__ */ new Date()).toISOString(),
          created_by: createdBy,
      ...
      case "receive_from_albert": {
        const { items } = body;
        if (!items || typeof items !== "object") return res.status(400).json({ error: "Missing items" });
        const movementIds = [];
        const { data: prices } = await supabase.from("laundry_prices").select("item_key, price").eq("active", true);
        const priceMap = {};
        for (const p of prices ?? []) priceMap[p.item_key] = Number(p.price);
        ...
        return res.json({ success: true, movement_ids: movementIds, calculated_cost, vat: vat3, total: calculated_cost + vat3 });
      }
      // ─── ASSIGN CLEANER ─────────────────────────────────────────────
      case "assign_cleaner": {
        const { slot_id, cleaner_name: cName, cleaner_telegram_id: cTgId, sub_apartment } = body;
        if (!slot_id) return res.status(400).json({ error: "Missing slot_id" });
        const { data: existing } = await supabase.from("cleaning_assignments").select("id, cleaner_name, status").eq("schedule_id", slot_id).neq("status", "cancelled").not("cleaner_name", "is", null);
        if (existing && existing.length > 0) {
          const taken = existing.find((a) => a.cleaner_name && normalizeCleanerName(a.cleaner_name) !== normalizeCleanerName(cName));
          if (taken) {
            return res.json({ error: `\u042D\u0442\u043E\u0442 \u0441\u043B\u043E\u0442 \u0443\u0436\u0435 \u0437\u0430\u043D\u044F\u0442: ${normalizeCleanerName(taken.cleaner_name)}` }, 409);
          }
        }
        const { data: slot, error: slotErr } = await supabase.from("cleaning_schedule").select("*").eq("id", slot_id).single();
        ...
        const { data: assignment, error: assignErr } = await supabase.from("cleaning_assignments").insert({
          schedule_id: slot_id,
          apartment: apt,
          cleaning_date: effectiveDate,
        ...
        return res.json({ success: true, data: normalizeAssignment(assignment) });
      }
      // ─── REPLACE CLEANER ─────────────────────────────────────────────
      case "replace_cleaner": {
        ...
        if (!refId) return res.status(400).json({ error: "Missing slot_id or assignment_id" });
        const record = await findAssignmentRecord(supabase, { assignment_id, schedule_id: slot_id });
        const normalized = normalizeCleanerName(cName);
        const tgId = normalizeTelegramId(cTgId) ?? await lookupCleanerChatId(supabase, cName);
        if (record) {
          await supabase.from("cleaning_assignments").update({ status: "cancelled" }).eq("id", record.id);
          if (record.cleaner_telegram_id) {
            const apt2 = APT_DISPLAY[record.apartment] ?? record.apartment;
            await notifyCleaner(record.cleaner_telegram_id, `\u274C \u0412\u0430\u0448\u0430 \u0443\u0431\u043E\u0440\u043A\u0430 ${apt2} ${fmtDate(record.cleaning_date)} \u043E\u0442\u043C\u0435\u043D\u0435\u043D\u0430. \u041D\u0430\u0437\u043D\u0430\u0447\u0435\u043D\u0430 \u0434\u0440\u0443\u0433\u0430\u044F \u0443\u0431\u043E\u0440\u0449\u0438\u0446\u0430.`);
          }
          const { data: newAssignment2, error: newErr2 } = await supabase.from("cleaning_assignments").insert({
          ...
          if (newErr2) throw newErr2;
          const aptDisplay = APT_DISPLAY[record.apartment] ?? record.apartment;
          await notifyAdmins(`\u{1F504} \u0417\u0430\u043C\u0435\u043D\u0430: ${aptDisplay} ${fmtDate(record.cleaning_date)}
${normalizeCleanerName(record.cleaner_name) ?? "\u2014"} \u2192 ${normalized}`);
          return res.json({ success: true, old: normalizeAssignment(record), data: normalizeAssignment(newAssignment2) });
        }
        ...
        if (newErr) throw newErr;
        return res.json({ success: true, data: normalizeAssignment(newAssignment) });
      }
      // ─── DELETE SLOT ──────────────────────────────────────────────────
      case "delete_slot": {
        const { slot_id } = body;
        if (!slot_id) return res.status(400).json({ error: "slot_id is required" });
        const { data: cancelled, error: cancelErr } = await supabase.from("cleaning_assignments").update({ status: "cancelled", cleaner_name: null, cleaner_telegram_id: null }).eq("schedule_id", slot_id).neq("status", "completed").select();
        if (cancelErr) throw cancelErr;
        return res.json({ success: true, cancelled_assignments: cancelled?.length ?? 0 });
      }
      // ─── GET BOOKING BY ID ──────────────────────────────────────────────
      case "get_booking_by_id": {
        const { booking_id } = body;
        if (!booking_id) return res.status(400).json({ error: "Missing booking_id" });
        const { data: booking, error: bookErr } = await supabase.from("cleaning_schedule").select("*").eq("id", booking_id).single();
        if (bookErr || !booking) return res.status(404).json({ error: "Booking not found" });
        const { data: assignment } = await supabase.from("cleaning_assignments").select("cleaner_name, cleaner_telegram_id, status").eq("schedule_id", booking_id).neq("status", "cancelled").maybeSingle();
        return res.json({
          success: true,
          data: {
            ...booking,
      ...
      case "create_albert_visit": {
        ...
        const delivered_cost = subtotal * 1.21;
        const { data: sentToAlbert } = await supabase.from("movements").select("item_type, quantity").eq("to_location", "albert_laundry");
        const { data: recvFromAlbert } = await supabase.from("movements").select("item_type, quantity").eq("from_location", "albert_laundry");
        const balance_after = {};
        ...
        for (const loc of ["dirty_linen_piral", "dirty_linen_salvador"]) {
          const { data: sentOut } = await supabase.from("movements").select("item_type, quantity").eq("from_location", loc);
          const { data: sentIn } = await supabase.from("movements").select("item_type, quantity").eq("to_location", loc);
          const locBalance = {};
        ...
        const { data: visit, error: visitErr } = await supabase.from("albert_visits").insert({
          visited_at: visited_at ?? (/* @__PURE__ */ new Date()).toISOString(),
      ...
      case "get_albert_visits": {
        ...
        const { data, error } = await supabase.from("albert_visits").select("id, visited_at, delivered_cost, delivered_items, picked_items, notes").order("visited_at", { ascending: false }).limit(limit);
        if (error) throw error;
      ...
      case "get_albert_visit_detail": {
        ...
        const { data, error } = await supabase.from("albert_visits").select("*").eq("id", visit_id).single();
        if (error || !data) return res.status(404).json({ error: "Visit not found" });
        return res.json({ success: true, data });
      }
      // ─── GET FINANCIAL BALANCE ──────────────────────────────────────────
      case "get_financial_balance": {
        const { data: invoices } = await supabase.from("laundry_invoices").select("invoice_amount");
        const total_invoiced = (invoices ?? []).reduce((s, i) => s + (Number(i.invoice_amount) || 0), 0);
        const { data: payments } = await supabase.from("emma_transactions").select("amount").eq("transaction_type", "expense").ilike("counterparty", "%\u0410\u043B\u044C\u0431\u0435\u0440\u0442%");
        const total_paid = (payments ?? []).reduce((s, p) => s + (Number(p.amount) || 0), 0);
        const { data: visits } = await supabase.from("albert_visits").select("delivered_cost");
      ...
      case "get_transactions_by_period": {
        ...
        return res.json({ success: true, data: allTx, summary: { total_income: totalIncome, total_expense: totalExpense, balance: totalIncome - totalExpense, count: allTx.length } });
      }
      // ─── 4. GET CLEANER STATS ───────────────────────────────────────────
      case "get_cleaner_stats": {
        const { cleaner_name, date_from, date_to } = body;
        if (!cleaner_name) return res.status(400).json({ error: "Missing cleaner_name" });
      ...
      case "get_laundry_cost_by_period": {
        const { date_from, date_to } = body;
        if (!date_from || !date_to) return res.status(400).json({ error: "Missing date_from/date_to" });
        const { data, error } = await supabase.from("movements").select("item_type, quantity, total_laundry_cost, laundry_item_cost, created_at").eq("to_location", "albert_laundry").gte("created_at", date_from).lte("created_at", date_to + "T23:59:59");
        if (error) throw error;
        ...
        return res.json({ success: true, data: { total_cost: totalCost, total_items: totalItems, movements_count: (data ?? []).length } });
      }
      // ─── 7. CREATE MOVEMENT ─────────────────────────────────────────────
      case "create_movement": {
        const { from_location, to_location, items, notes } = body;
        if (!from_location || !to_location || !items) return res.status(400).json({ error: "Missing from_location/to_location/items" });
        const locationMap = {
      ...
      case "get_transactions_by_category": {
        ...
        if (!cat || !date_from || !date_to) return res.status(400).json({ error: "Missing category/date_from/date_to" });
        let all = [];
        ...
        return res.json({ success: true, data: all, total: all.reduce((s, t) => s + Number(t.amount), 0) });
      }
      ...
      case "get_income_sources": {
        ...
        return res.json({ success: true, data: bySource, total: all.reduce((s, t) => s + Number(t.amount), 0) });
      }
      ...
      case "get_unpaid_cleanings": {
        const { data, error } = await supabase.from("cleaning_assignments").select("cleaner_name, cleaning_date, apartment, payment_amount, status").in("status", ["completed", "assigned", "started"]).eq("payment_confirmed", false).order("cleaning_date", { ascending: true });
        if (error) throw error;
        const byName = {};
        for (const r of data ?? []) {
      ...
      case "get_stock_balance": {
        ...
        if (location) {
          return res.json({ success: true, data: { [location]: balance[location] || {} } });
        }
      ...
      case "update_task_status": {
        ...
        if (!task_id || !newStatus) return res.status(400).json({ error: "Missing task_id/status" });
      ...
      case "get_bookings_summary": {
        ...
        for (const r of rows) {
          ...
          if (r.checkin_date && r.checkout_date) {
            totalDuration += Math.round((new Date(r.checkout_date).getTime() - new Date(r.checkin_date).getTime()) / 864e5);
          }
        }
        const { data: incomes } = await supabase.from("main_transactions").select("amount").eq("transaction_type", "income").gte("transaction_date", date_from).lte("transaction_date", date_to + "T23:59:59");
        const totalIncome = (incomes ?? []).reduce((s, t) => s + Number(t.amount), 0);
        const periodDays = Math.max(1, Math.round((new Date(date_to).getTime() - new Date(date_from).getTime()) / 864e5));
      ...
      case "get_apartment_revenue": {
        ...
        return res.json({ success: true, data: { ...result, unmatched }, total: Object.values(result).reduce((s, v) => s + v, 0) + unmatched });
      }
      ...
      case "get_albert_debt_history": {
        ...
        return res.json({ success: true, data: history, current_debt: runningDebt });
      }
      // ─── GET BOOKINGS WITHOUT INCOME ─────────────────────────────────
      case "get_bookings_without_income": {
        const today = (/* @__PURE__ */ new Date()).toISOString().split("T")[0];
        const { data: bookings, error } = await supabase.from("cleaning_schedule").select("id, apartment, checkin_date, checkout_date, guests_count, source, gap_days").lte("checkout_date", today).order("checkout_date", { ascending: false }).limit(200);
        if (error) throw error;
        const { data: incomes } = await supabase.from("main_transactions").select("description, location").eq("transaction_type", "income");
        const aptKwMap = {
          piral_1: ["\u043E\u0430\u0437\u0438\u0441 1", "oasis 1", "oasis1", "piral_1"],
          piral_2: ["\u043E\u0430\u0437\u0438\u0441 2", "oasis 2", "oasis2", "piral_2"],
          grande: ["\u0433\u0440\u0430\u043D\u0434\u0435", "grande", "oasis_grande"],
        ...
        const { data: assignments } = await supabase.from("cleaning_assignments").select("schedule_id, cleaner_name").in("schedule_id", bookingIds.length > 0 ? bookingIds : ["00000000-0000-0000-0000-000000000000"]);
        const cleanerMap = {};
      ...
      case "upload_receipt": {
        ...
        if (!image_base64 || !file_name) return res.status(400).json({ error: "Missing image_base64 or file_name" });
        ...
        const { error: uploadError } = await supabase.storage.from("receipts").upload(path, bytes, { contentType, upsert: false });
      ...
      case "log_lock_code": {
        ...
        if (!apartment || !code) return res.status(400).json({ error: "apartment and code are required" });
        const { data: entry, error: lcErr } = await supabase.from("lock_codes_log").insert({
          apartment,
          code,
      ...
      case "log_guest_message": {
        ...
        if (!apartment) return res.status(400).json({ error: "apartment is required" });
        const { data: entry, error: gmErr } = await supabase.from("guest_messages_log").insert({
          apartment,
          code: code ?? null,
        ...
        if (gmErr) return res.status(500).json({ error: gmErr.message });
        return res.json({ success: true, data: entry });
      }
      case "update_expense_receipt": {
        ...
        const updateFields = {};
        if (receipt_url !== void 0) updateFields.receipt_url = receipt_url;
        if (receipt_text !== void 0) updateFields.receipt_text = receipt_text;
        if (amount !== void 0) updateFields.amount = amount;
        if (description !== void 0) updateFields.description = description;
        if (Object.keys(updateFields).length === 0) {
          return res.status(400).json({ error: "No fields to update" });
        }
        const { data: updated, error: updErr } = await supabase.from("emma_transactions").update(updateFields).eq("id", expense_id).select().single();
      ...
      case "create_guest_portal": {
        ...
        if (!apartment || !checkin_date || !checkout_date) {
          return res.json({ error: "apartment, checkin_date, checkout_date required" }, 400);
        }
        const chars = "ABCDEFGHJKLMNPQRSTUVWXYZ23456789";
        let token = "";
        for (let i = 0; i < 8; i++) token += chars[Math.floor(Math.random() * chars.length)];
        token = token.toLowerCase();
        const { data: portal, error: pErr } = await supabase.from("guest_portals").insert({
          token,
          apartment,
          checkin_date,
          checkout_date,
        ...
        return res.json({ success: true, token, url, portal });
      }
      case "get_guest_portal": {
        ...
        const { data: portal, error: gErr } = await supabase.from("guest_portals").select("*").eq("token", token).single();
        if (gErr || !portal) return res.json({ error: "Portal not found", expired: true }, 404);
        if (portal.checkin_date > tomorrowStr || portal.checkout_date < todayStr || portal.status === "expired") {
          return res.json({ expired: true });
        }
        return res.json({ expired: false, portal });
      }
      default:

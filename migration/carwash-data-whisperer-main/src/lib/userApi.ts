...
const FUNCTION_URL = `https://wjvsdpgwhdriftevxsqp.supabase.co/functions/v1/manage-users`;
...
export function isAdminUser(): boolean {
  return localStorage.getItem('carwash_user') === 'georgen77' && getUserRole() === 'admin';
}
...
export async function registerBiometric(userId: string, deviceName?: string) {
  const { challenge } = await callManageUsers({ action: 'webauthn_register_challenge', userId });
  ...
  // Store credential ID locally for allowCredentials hint on auth
...
// Convert base64url → standard base64 for atob()
...
export async function authenticateWithBiometric(): Promise<{ success: boolean; token?: string; username?: string; role?: string; fullName?: string; userId?: string; error?: string }> {
  try {
    // Get stored credential IDs to pass as allowCredentials hint
    ...
    // Use both rawId and id forms for matching
    const credentialIdRaw = btoa(String.fromCharCode(...new Uint8Array(credential.rawId)));
    const credentialIdBase64url = credential.id;
    
    return callManageUsers({ action: 'webauthn_authenticate', credentialId: credentialIdRaw, credentialIdAlt: credentialIdBase64url });
  } catch (e) {

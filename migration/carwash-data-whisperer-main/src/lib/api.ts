...
export async function fetchDetails(): Promise<DetailsResponse> {
  const { data, error } = await supabase.functions.invoke('scrape-carwash', {
    body: { reportType: 'details', dateFrom: '2026-01-01', dateTo: '2026-01-01', authToken: getAuthToken() },
  });
...
export async function fetchWashDetails(washIdx: number): Promise<DetailsResult> {
  const { data, error } = await supabase.functions.invoke('scrape-carwash', {
    body: { reportType: 'details', washIdx, dateFrom: '2026-01-01', dateTo: '2026-01-01', authToken: getAuthToken() },
  });
  if (error) throw new Error(error.message);
...
function getAuthToken(): string {
  // Always return btoa("username:timestamp") format required by scrape-carwash verifyAuth
  const username = localStorage.getItem('carwash_user');
  if (username) {
    // Regenerate each call so token is always fresh and valid
    return btoa(`${username}:${Date.now()}`);
  }
  // Last resort: legacy token (may be stale but better than nothing)
...
export async function login(username: string, password: string): Promise<{ success: boolean; error?: string }> {
  ...
  if (data.success) {
    ...
    localStorage.setItem('carwash_legacy_token', data.token); // legacy format for scrape-carwash
...
async function fetchChunk(dateFrom: string, dateTo: string, retries = 2): Promise<AllWashesResponse> {
  ...
  if (dateTo < today) {
    ...
    if (cached?.report_data) {
      return cached.report_data as unknown as AllWashesResponse;
    }
  }
  ...
  for (let attempt = 0; attempt <= retries; attempt++) {
    try {
      const { data, error } = await supabase.functions.invoke('scrape-carwash', {
        body: { washIndex: 'all', dateFrom, dateTo, authToken: getAuthToken() },
      });
      if (error) throw new Error(error.message);

      if (dateTo < today && data?.success) {
        await supabase.from('report_cache').upsert(
          { cache_key: cacheKey, report_data: data },
...
// Split large date ranges into monthly chunks to avoid edge function timeouts
...
function mergeResults(responses: AllWashesResponse[], dateFrom: string, dateTo: string): AllWashesResponse {
  ...
  const merged: WashReport[] = washNames.map(name => {
    ...
    // Sum totalRow values (index 1+ are numbers)
    const totalRow = allForWash[0]?.totalRow?.map((val, i) => {
      if (i === 0) return val; // label "Підсумок"
...
export async function fetchAllWashes(
  ...
  // If ≤31 days, single request
  if (days <= 31) {
    return fetchChunk(dateFrom, dateTo);
  }

  // Split into monthly chunks and fetch in parallel (max 6 concurrent)
...
export async function fetchExtraReport(reportType: ReportType, dateFrom: string, dateTo: string): Promise<ExtraReportResponse> {
  const { data, error } = await supabase.functions.invoke('scrape-carwash', {
    body: { reportType, dateFrom, dateTo, authToken: getAuthToken() },
  });
  if (error) throw new Error(error.message);
...
export async function restoreExpense(log: ExpenseLog): Promise<void> {
  ...
  if (log.action === 'delete') {
    ...
    if (error) throw new Error(error.message);
    // Log the restore action (store original action date in new_data)
    await supabase.from('expense_logs').insert({
      expense_id: data.id,
      action: 'restore',
      changed_by: getUsername(),
...
export async function fetchCardPayments(washName: string, dateFrom: string, dateTo: string): Promise<{ headers: string[]; rows: string[][]; totalRow: string[] }> {
  const { data, error } = await supabase.functions.invoke('scrape-carwash', {
    body: { reportType: 'cardPayments', washName, dateFrom, dateTo, authToken: getAuthToken() },
  });
  if (error) throw new Error(error.message);
  return data;
}
...
/**
 * Calculate expected monthly default expenses for a given period.
 * Returns total and breakdown by expense_type and by wash.
 */
export function calcMonthlyDefaults(
  ...
  for (const monthDate of months) {
    ...
    for (const def of defaults) {
      // Check if this default applies to this month
      ...
      // Proportional amount if period doesn't cover full month
